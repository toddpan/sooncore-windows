...
CPartiaipantList::~CPartiaipantList()
{
	if (root_node_)
		delete root_node_;
...
bool CPartiaipantList::Add(CControlUI* pControl)
{
	if (!pControl)
		return false;
...
bool CPartiaipantList::AddAt(CControlUI* pControl, int iIndex)
{
	if (!pControl)
		return false;
...
bool CPartiaipantList::Remove(CControlUI* pControl)
{
	if (!pControl)
		return false;
...
bool CPartiaipantList::RemoveAt(int iIndex)
{
...
void CPartiaipantList::RemoveAll()
{
	CListUI::RemoveAll();
...
void CPartiaipantList::DoEvent(TEventUI& event) 
{
...
Node* CPartiaipantList::AddNode(const PtiptListItemInfo& item, Node* parent)
{
	if (!parent)
		parent = root_node_;
	...
	if (video_button != NULL)
	{
		...
		//取消打开聊天窗口事件
		video_button->OnEvent += MakeDelegate(&OnVideoButtonEvent);
	}
...
bool CPartiaipantList::RemoveNode(Node* node)
{
...
void CPartiaipantList::SetChildVisible(Node* node, bool visible)
{
...
bool CPartiaipantList::CanExpand(Node* node) const
{
	if (!node || node == root_node_)
		return false;
